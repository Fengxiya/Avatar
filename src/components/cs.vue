<template>
    <div class="weather-box">
      <img
        class="position-icon"
        src="../assets/weather/position.svg"
      />
      <div class="position-province">{{ userProvince }}</div>
      <div class="position-city">{{ userCity }}</div>
      <div class="position-weather">{{ nowWeather }}</div>
      <!-- <img
        class="weather-icon"
        src="../assets/weather/100.svg"
      /> -->
      <!-- 这个是显示天气图标的部分 -->
      <div class="weather">
        <div v-if="weatherType == 'sunny'">
          <div class="sunny">
            <span class="sun"></span>
          </div>
        </div>
        <div v-if="weatherType == 'cloudy'">
          <div class="cloudy">
            <span class="cloud"></span>
            <span class="cloud"></span>
          </div>
        </div>
        <div v-if="weatherType == 'snowy'">
          <div class="snowy">
  ...
  async function getPosition() {
    ...
    if (posRes.message == "Success") {
      ...
      // 将ip存到localstorage中
      if (localStorage.getItem("userAvatarIP")) {
        let oldIP = localStorage.getItem("userAvatarIP")
        // 拿到旧的IP和新的ip做对比，但是下面的这个老是报错
        // let myOldIP = JSON.parse(oldIP)
      } else {
        // 之前浏览器的localstorage中不存在avatar项目存进去的IP,就把用户最新的IP存进去
        window.localStorage.setItem("userAvatarIP", JSON.stringify(userIP.value))
      }
